...
IV_test<-data.frame(c(seq(0.1, 2, by=0.1),seq(2, 10, by=1)))
colnames(IV_test)=c("IV")
...
for (IV_loop in 1:nrow(IV_test)){
    ...
    
  {
    ...
    strength_IV=IV_test$IV[IV_loop]  
...
for (i in 1:exp_num)
{
  for (j in 1:snp_num)
  {
    data[,paste0('a_sub_snp', i,j)]<-(data[,paste0('snp', j)]+rnorm(snp_num, mean = 0, sd = strength_IV))*lmk_x[i,j]
  }
}
...
IV_test$meanF[IV_loop]<-mean_F123

IV_test$p1_mean[IV_loop]<-MMR_v1_mean
IV_test$p2_mean[IV_loop]<-MMR_v2_mean
IV_test$p3_mean[IV_loop]<-MMR_v3_mean

IV_test$p1_se[IV_loop]<-MMR_v1_std
IV_test$p2_se[IV_loop]<-MMR_v2_std
IV_test$p3_se[IV_loop]<-MMR_v3_std

IV_test$p1_cover[IV_loop]<-MMR_v1_coverage
IV_test$p2_cover[IV_loop]<-MMR_v2_coverage
IV_test$p3_cover[IV_loop]<-MMR_v3_coverage

IV_test$p1_power[IV_loop]<-MMR_v1_power
IV_test$p2_power[IV_loop]<-MMR_v2_power 
IV_test$p3_power[IV_loop]<-MMR_v3_power

IV_test$p1_FPR[IV_loop]<-MMR_v1_fpr
IV_test$p2_FPR[IV_loop]<-MMR_v2_fpr
IV_test$p3_FPR[IV_loop]<-MMR_v3_fpr

}



write.csv(IV_test,"./results/Figure2/UBP.csv")
